//! clawdpanel-media — station-source parsing for the Rust/Slint rewrite.
//!
//! A 1:1 port of Go's `internal/station/parse.go`: turn a user-pasted YouTube
//! URL or bare ID into a typed [`StationItem`], and report whether a station can
//! ever step through more than one track. The settings editor uses [`parse_item`]
//! for server-side validation (the old `ParseStationItem` binding); the bar uses
//! [`has_multiple_tracks`] to gate its track-skip buttons.
