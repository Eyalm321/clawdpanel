...
impl Video {
    ...
    pub async fn get_basic_info(&self) -> Result<VideoInfo, VideoError> {
        ...
        // Force fallback to bypass PO Token requirements
        // Force fallback to bypass PO Token requirements
        {
            ...
            if let Some(client_cfg) = INNERTUBE_CLIENT.get("android") {
                if let Ok(config_str) = self.get_player_ytconfig(&response, client_cfg.clone()).await {
                    ...
                    if let Some(ref sd) = player_response_new.streaming_data {
                        if sd.dash_manifest_url.is_some() || sd.hls_manifest_url.is_some() || sd.adaptive_formats.is_some() {
                            let orig_dash = player_response.streaming_data.as_ref().and_then(|x| x.dash_manifest_url.clone());
                            let orig_hls = player_response.streaming_data.as_ref().and_then(|x| x.hls_manifest_url.clone());
                            
                            player_response.streaming_data = Some(sd.clone());
                            
                            if let Some(ref mut new_sd) = player_response.streaming_data {
                                if new_sd.dash_manifest_url.is_none() && orig_dash.is_some() {
                                    new_sd.dash_manifest_url = orig_dash;
                                }
                                if new_sd.hls_manifest_url.is_none() && orig_hls.is_some() {
                                    new_sd.hls_manifest_url = orig_hls;
                                }
                            }
            ...
            // Try ios client if tv/android failed
            if !fallback_success {
                if let Some(client_cfg) = INNERTUBE_CLIENT.get("ios") {
                    if let Ok(config_str) = self.get_player_ytconfig(&response, client_cfg.clone()).await {
                        let player_response_new = serde_json::from_str::<PlayerResponse>(&config_str).unwrap_or_default();
                        if let Some(ref sd) = player_response_new.streaming_data {
                            let orig_dash = player_response.streaming_data.as_ref().and_then(|x| x.dash_manifest_url.clone());
    ...
    async fn get_player_ytconfig(
        ...
        if configs.1 == "5" {
            headers.insert(
                ...
                HeaderValue::from_str("com.google.ios.youtube/21.02.3 (iPhone16,2; U; CPU iOS 18_3_2 like Mac OS X;)").unwrap(),
            );
        } else if configs.1 == "3" {
            headers.insert(
                HeaderName::from_str("User-Agent").unwrap(),
                HeaderValue::from_str("com.google.android.youtube/21.02.35 (Linux; U; Android 11) gzip").unwrap(),
            );
        } else if configs.1 == "7" {
            headers.insert(
                HeaderName::from_str("User-Agent").unwrap(),
                HeaderValue::from_str("Mozilla/5.0 (ChromiumStylePlatform) Cobalt/25.lts.30.1034943-gold (unlike Gecko), Unknown_TV_Unknown_0/Unknown (Unknown, Unknown)").unwrap(),
            );
        }
